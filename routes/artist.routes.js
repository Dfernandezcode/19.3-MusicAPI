...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    if (artist) {
      ...
      if (includePlaylists) {
        const playlist = await Playlist.find({ artist: id }); // Busco en la entidad Car los coches que correspondena ese id de User.
        temporalArtist.playlist = playlist; // Añadimos la propiedad cars al usuario temporal con los coches que hemos recogido de la entidad Car.
      }
      res.json(temporalArtist); //  Si existe el artist lo mandamos como respuesta en modo json.
    } else {
